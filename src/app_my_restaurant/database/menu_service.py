...
def agregar_menu_item_db(nombre, descripcion, precio, tipo_id, estado=1):
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO menu_items (nombre, descripcion, precio, tipo, estado) VALUES (?, ?, ?, ?, ?)",
                (nombre, descripcion, precio, tipo_id, estado)
            )
        return True, f"Ítem '{nombre}' agregado con éxito."
    except Exception as e:
        return False, f"Error al agregar ítem '{nombre}': {e}"

def actualizar_menu_item_db(item_id, nombre, descripcion, precio, tipo_id, estado):
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE menu_items SET nombre = ?, descripcion = ?, precio = ?, tipo_id = ?, estado = ? WHERE id = ?",
                (nombre, descripcion, precio, tipo_id, estado, item_id)
            )
