...
def eliminar_tipo_menu_db(tipo_id):
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM menu_tipos WHERE id = ?", (tipo_id,))
            return True, f"Tipo eliminado correctamente. ID: {tipo_id}"
    except Exception as e:
        return False, str(e)

def actualizar_tipo_menu_db(tipo_id, nuevo_nombre, estado):
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("UPDATE menu_tipos SET nombre = ?, estado = ? WHERE id = ?", (nuevo_nombre, int(estado), tipo_id))
